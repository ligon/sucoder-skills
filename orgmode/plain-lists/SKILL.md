...
6. **Paragraph Continuation** — For multi-paragraph list items, indent subsequent lines to align under the text—not under the bullet marker. The same indentation carries the sentence-level line breaks required by `orgmode/semantic-line-breaks`: the item's first sentence shares the bullet line (`- First sentence.`, or `- Term :: First sentence.`), and each later sentence starts its own line at the text column.
...
- `orgmode/semantic-line-breaks` — One sentence per line, including inside list items.
